...
export const useUserStore = defineStore("users", () => {
  ...
  const getProjectMember = async (projectId: number) => {
    try {
      ...
      if (res.success && res.data) {
        // 将后端返回的项目成员数据转换为前端 UserItem 格式
        const transformedMembers: UserItem[] = [];
        const members = res.data as any[];

        // 并行获取所有成员的详细信息
        const memberDetailsPromises = members.map(async (member: any) => {
          try {
            const userRes = await fetchUserById(member.user_id);
            if (userRes.success && userRes.data) {
              return {
                ...transformBackendUser(userRes.data),
                status: member.is_active ? "online" : "offline",
                postion: member.position || defaultUser.postion,
                percentage: 0,
                role: member.role || defaultUser.role,
              };
            }
          } catch (e) {
            console.log(`获取成员 ${member.user_id} 详细信息失败`, e);
          }
          // 如果获取失败，返回基本数据
          return {
            ...defaultUser,
        ...
        // 等待所有请求完成
        const results = await Promise.all(memberDetailsPromises);
        transformedMembers.push(
          ...results.filter((item): item is UserItem => item !== undefined),
        );

        // 替换静态数据 usersTable
        usersTable.splice(0, usersTable.length, ...transformedMembers);

        console.log("项目成员数据加载完成:", transformedMembers);
      }
    } catch (e) {
      console.log("获取项目成员失败", e);
    }
  };
  // 初始化：从登录状态加载用户数据
  const initUser = () => {
